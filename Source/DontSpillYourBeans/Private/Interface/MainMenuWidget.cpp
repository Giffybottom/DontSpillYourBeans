...
void UMainMenuWidget::BindButtons()
{
	...
	if (GamesInstance != nullptr)
	{
		...
		//
		UNewSaveMenuWidget* newgamewidget = Cast <UNewSaveMenuWidget >(MenuSwitcher->GetWidgetAtIndex(1));
		if (newgamewidget != nullptr)
		{
			newgamewidget->buttonConfirm->buttonWTXT->OnClicked.AddDynamic(this, &UMainMenuWidget::NewGame);
		}
		...
		if (Loadwidget !=nullptr)
		{
			...
			// toggle continue and load btns visiblity
		}
...
void UMainMenuWidget::NewGame()
{
	...
	{
		UNewSaveMenuWidget* NewgameMenu = Cast <UNewSaveMenuWidget>(MenuSwitcher->GetChildAt(1));
		FText saveName = NewgameMenu->EditableTextBoxInput->GetText();

		if (GamesInstance->MakeNewSave(saveName.ToString()))
		{
